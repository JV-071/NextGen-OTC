...
Ported from OpenTibiaBR/otclient, commit
`9bfac7719fd5cd2d8a2cddf2ea6219e908e129f9`, by Renan Miyashiro (MIT).
The NextGen engine exposes `g_resources.writeFileContentsToWorkDir` for saving
loose project files, restoring the settings write directory afterwards.
Rebuild the executable before using this module. Files embedded in `assets.pak`
are not editable in place; use a development checkout with loose assets.
Headless checks: `lua tests/lua/dev_otui_test.lua` from the repository root.
